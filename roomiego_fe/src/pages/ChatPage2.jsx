...
const ChatPage2 = () => {
  ...
  const onPrivateMessage = (payload) => {
    ...
    if (payloadData.status === 'MESSAGE') {
      const chatKey = payloadData.senderName === username
  ...
  const fetchChatHistory = async (user1, user2) => {
    ...
    try {
      ...
      const response = await axios.get(
        `http://localhost:8080/messages/api/messages/history/${user1}/${user2}`,
        {
          headers: {
      ...
      if (response.status === 200) {
        setPrivateChats((prevChats) => {
          prevChats.set(user2, response.data.filter(msg => msg.status === 'MESSAGE'));
